function [cineq,ceq] = nonlinear(u1,u1s,x1,x2,x3,x1s,x2s,x3s,x7,x8,x9,x7s,x8s,x9s,Delta,interval,pred,u1pred,F1,FR,Ff1,V1,alA,alB,alC,kA,kB,EAR,EBR,dHA,dHB,Cp,T0,xA0)
...
%% nonlinear
    ...
    g9=1/(Cp*V3);
    alpha_3 = 0.5;
    V = (x(1)-xs(1))^2 +  (x(2)-xs(2))^2 +0.0001*(x(3)-xs(3))^2 + (x(4)-xs(4))^2 + (x(5)-xs(5))^2 +0.0001*(x(6)-xs(6))^2 + (x(7)-xs(7))^2 + (x(8)-xs(8))^2 + 0.0001*(x(9)-xs(9))^2;
    dV = 2*[x(1)-xs(1);x(2)-xs(2);(x(3)-xs(3))*0.0001;x(4)-xs(4);x(5)-xs(5);0.0001*(x(6)-xs(6));x(7)-xs(7);x(8)-xs(8);(x(9)-xs(9))*0.0001];
    Lf = dV' * [f1;f2;f3;f4;f5;f6;f7;f8;f9];
    Lg = dV' * [0;0;g3*u(1);0;0;g6*u(2);0;0;g9*u(3)];
    cineq = Lf + Lg + alpha_3 * V;
